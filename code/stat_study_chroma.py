...
#tm successful it0
rms_beta_x6=np.empty(0)
ave_beta_x6=np.empty(0)
rms_beta_y6=np.empty(0)
ave_beta_y6=np.empty(0)

#tm all it0
rms_beta_x7 =np.empty(0)
ave_beta_x7 =np.empty(0)
rms_beta_y7 =np.empty(0)
ave_beta_y7 =np.empty(0)

#tm successful it1
rms_beta_x8 =np.empty(0)
ave_beta_x8 =np.empty(0)
rms_beta_y8 =np.empty(0)
ave_beta_y8 =np.empty(0)

#tm all it1
rms_beta_x9 =np.empty(0)
ave_beta_x9 =np.empty(0)
rms_beta_y9 =np.empty(0)
ave_beta_y9 =np.empty(0)
...
for i in range(eseed):
    ...
    if os.path.exists(fnameall5):
        ...
        rms_beta_x5=np.append(rms_beta_x5,np.std(100.*((optics_all5['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        ave_beta_x5=np.append(ave_beta_x5,np.mean(100.*((optics_all5['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        rms_beta_y5=np.append(rms_beta_y5,np.std(100.*((optics_all5['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
        ave_beta_y5=np.append(ave_beta_y5,np.mean(100.*((optics_all5['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
        ...
        #ax11[0].set_ylim(-20,20) #200um
        #ax11[0].set_ylim(-10,10)
        ...
        #ax11[1].set_ylim(-60,60) #200um
        #ax11[1].set_ylim(-40,40)
    ...
    if os.path.exists(fnameall6):
        ...
        if round(Q_x,3)==round(Q1,3) and round(Q_y,2)==round(Q2,2):
            ...
            rms_beta_x6=np.append(rms_beta_x6,np.std(100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
            ave_beta_x6=np.append(ave_beta_x6,np.mean(100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
            rms_beta_y6=np.append(rms_beta_y6,np.std(100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
            ave_beta_y6=np.append(ave_beta_y6,np.mean(100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))

            ax17[0].plot(optics_all6['S']/1000., 100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"]), ".")
            ax17[0].set_ylabel(r"$\Delta \beta_{x }/ \beta_{x,ref}$ [%]")
            ax17[0].set_ylim(-10,10)
            ax17[1].plot(optics_all6['S']/1000., 100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"]), ".")
            ax17[1].set_xlabel("longitundinal position [km]")
            ax17[1].set_ylabel(r"$\Delta \beta_{y} / \beta_{y,ref}$ [%]")
            ax17[1].set_ylim(-40,40)

            tts+=1

        xseed7=np.append(xseed7,mms)
        rms_beta_x7=np.append(rms_beta_x7,np.std(100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        ave_beta_x7=np.append(ave_beta_x7,np.mean(100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        rms_beta_y7=np.append(rms_beta_y7,np.std(100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
        ave_beta_y7=np.append(ave_beta_y7,np.mean(100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))

        ax18[0].plot(optics_all6['S']/1000., 100.*((optics_all6['BETX']-optics_ref["BETX"])/optics_ref["BETX"]), ".")
        ax18[0].set_ylabel(r"$\Delta \beta_{x }/ \beta_{x,ref}$ [%]")
        ax18[0].set_ylim(-10,10)
        ax18[1].plot(optics_all6['S']/1000., 100.*((optics_all6['BETY']-optics_ref["BETY"])/optics_ref["BETY"]), ".")
        ax18[1].set_xlabel("longitundinal position [km]")
        ax18[1].set_ylabel(r"$\Delta \beta_{y} / \beta_{y,ref}$ [%]")
        ax18[1].set_ylim(-40,40)
    ...
    if os.path.exists(fnameall7):
        ...
        if round(Q_x,3)==round(Q1,3) and round(Q_y,2)==round(Q2,2):
            ...
            rms_beta_x8=np.append(rms_beta_x8,np.std(100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
            ave_beta_x8=np.append(ave_beta_x8,np.mean(100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
            rms_beta_y8=np.append(rms_beta_y8,np.std(100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
            ave_beta_y8=np.append(ave_beta_y8,np.mean(100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))

            ax21[0].plot(optics_all7['S']/1000., 100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"]), ".")
            ax21[0].set_ylabel(r"$\Delta \beta_{x }/ \beta_{x,ref}$ [%]")
            ax21[0].set_ylim(-10,10)
            ax21[1].plot(optics_all7['S']/1000., 100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"]), ".")
            ax21[1].set_xlabel("longitundinal position [km]")
            ax21[1].set_ylabel(r"$\Delta \beta_{y} / \beta_{y,ref}$ [%]")
            ax21[1].set_ylim(-40,40)

            ffs+=1

        xseed9=np.append(xseed9,dds)
        rms_beta_x9=np.append(rms_beta_x9,np.std(100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        ave_beta_x9=np.append(ave_beta_x9,np.mean(100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"])))
        rms_beta_y9=np.append(rms_beta_y9,np.std(100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))
        ave_beta_y9=np.append(ave_beta_y9,np.mean(100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"])))

        ax22[0].plot(optics_all7['S']/1000., 100.*((optics_all7['BETX']-optics_ref["BETX"])/optics_ref["BETX"]), ".")
        ax22[0].set_ylabel(r"$\Delta \beta_{x }/ \beta_{x,ref}$ [%]")
        ax22[0].set_ylim(-10,10)
        ax22[1].plot(optics_all7['S']/1000., 100.*((optics_all7['BETY']-optics_ref["BETY"])/optics_ref["BETY"]), ".")
        ax22[1].set_xlabel("longitundinal position [km]")
        ax22[1].set_ylabel(r"$\Delta \beta_{y} / \beta_{y,ref}$ [%]")
        ax22[1].set_ylim(-40,40)
...
fig3.savefig(path+'beta_distribution_line_sextuoff_it1_{0}seeds.png'.format(eseed))
fig5.savefig(path+'beta_distribution_line_sextuoff_it2_{0}seeds.png'.format(eseed))
fig9.savefig(path+'beta_distribution_ring_sextuoff_first_it_{0}seeds.png'.format(eseed))
fig10.savefig(path+'beta_distribution_ring_sextuoff_last_it_{0}seeds.png'.format(eseed))
fig11.savefig(path+'beta_distribution_ring_sextuon_it1_{0}seeds.png'.format(eseed))
fig17.savefig(path+'beta_distribution_tm_seccessful_it0_{0}seeds.png'.format(eseed))
fig18.savefig(path+'beta_distribution_tm_all_it0_{0}seeds.png'.format(eseed))
fig21.savefig(path+'beta_distribution_tm_seccessful_it1_{0}seeds.png'.format(eseed))
fig22.savefig(path+'beta_distribution_tm_all_it1_{0}seeds.png'.format(eseed))

#plot of the rms: all
fig17, ax17=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax17[0].plot(xseed, rms_beta_x, ".", label='MS_off_line: it1')
ax17[0].plot(xseed2, rms_beta_x2, ".", label='MS_off_line: it2')
ax17[0].plot(xseed3, rms_beta_x3, ".", label='MS_off_ring: first it')
ax17[0].plot(xseed4, rms_beta_x4, ".", label='MS_off_ring: last it')
ax17[0].plot(xseed5, rms_beta_x5, ".", label='MS_on_ring')
ax17[0].set_ylabel("rms$_x$ [%]")
ax17[0].legend(fontsize=8,loc='best')
ax17[1].plot(xseed, rms_beta_y, ".", label='MS_off_line: it1')
ax17[1].plot(xseed2, rms_beta_y2, ".", label='MS_off_line: it2')
ax17[1].plot(xseed3, rms_beta_y3, ".", label='MS_off_ring: first it')
ax17[1].plot(xseed4, rms_beta_y4, ".", label='MS_off_ring: last it')
ax17[1].plot(xseed5, rms_beta_y5, ".", label='MS_on_ring')
...
fig18, ax18=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax18[0].plot(xseed, ave_beta_x, ".", label='MS_off_line: it1')
ax18[0].plot(xseed2, ave_beta_x2, ".", label='MS_off_line: it2')
ax18[0].plot(xseed3, ave_beta_x3, ".", label='MS_off_ring: first it')
ax18[0].plot(xseed4, ave_beta_x4, ".", label='MS_off_ring: last it')
ax18[0].plot(xseed5, ave_beta_x5, ".", label='MS_on_ring')
ax18[0].set_ylabel("mean$_x$ [%]")
ax18[0].legend(fontsize=8,loc='best')
ax18[1].plot(xseed, ave_beta_y, ".", label='MS_off_line: it1')
ax18[1].plot(xseed2, ave_beta_y2, ".", label='MS_off_line: it2')
ax18[1].plot(xseed3, ave_beta_y3, ".", label='MS_off_ring: first it')
ax18[1].plot(xseed4, ave_beta_y4, ".", label='MS_off_ring: last it')
ax18[1].plot(xseed5, ave_beta_y5, ".", label='MS_on_ring')
...
fig2.savefig(path+'rms_beta_line_sextuoff_{0}seeds.pdf'.format(eseed))
...
fig4.savefig(path+'mean_beta_line_sextuoff_{0}seeds.pdf'.format(eseed))

#plot of the rms: sextuoff/on ring
fig12, ax12=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax12[0].plot(xseed3, rms_beta_x3, ".", label='MS_off: first it')
ax12[0].plot(xseed4, rms_beta_x4, ".", label='MS_off: last it')
ax12[0].plot(xseed5, rms_beta_x5, ".", label='MS_on')
ax12[0].set_ylabel("rms$_x$ [%]")
ax12[0].legend(fontsize=8,loc='best')
ax12[1].plot(xseed3, rms_beta_y3, ".", label='MS_off: first it')
ax12[1].plot(xseed4, rms_beta_y4, ".", label='MS_off: last it')
ax12[1].plot(xseed5, rms_beta_y5, ".", label='MS_on')
...
fig12.savefig(path+'rms_beta_ring_{0}seeds.pdf'.format(eseed))

#plot of the mean: sextuoff/on ring
fig13, ax13=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax13[0].plot(xseed3, ave_beta_x3, ".", label='MS_off: first it')
ax13[0].plot(xseed4, ave_beta_x4, ".", label='MS_off: last it')
ax13[0].plot(xseed5, ave_beta_x5, ".", label='MS_on')
ax13[0].set_ylabel("mean$_x$ [%]")
ax13[0].legend(fontsize=8,loc='best')
ax13[1].plot(xseed3, ave_beta_y3, ".", label='MS_off: first it')
ax13[1].plot(xseed4, ave_beta_y4, ".", label='MS_off: last it')
ax13[1].plot(xseed5, ave_beta_y5, ".", label='MS_on')
...
fig13.savefig(path+'mean_beta_ring_{0}seeds.pdf'.format(eseed))

#plot of the rms: tm without svd
fig20, ax20=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax20[0].plot(xseed6, rms_beta_x6, ".", label='tm success')
ax20[0].plot(xseed7, rms_beta_x7, ".", label='tm all')
ax20[0].set_ylabel("mean$_x$ [%]")
ax20[0].legend(fontsize=8,loc='best')
ax20[1].plot(xseed6, rms_beta_y6, ".", label='tm success')
ax20[1].plot(xseed7, rms_beta_y7, ".", label='tm all')
ax20[1].set_xlabel("seed")
ax20[1].set_ylabel("mean$_x$ [%]")
ax20[1].legend(fontsize=8,loc='best')
fig20, plt.subplots_adjust(left=.16, right=.97, top=.94, bottom=.11)
fig20.savefig(path+'rms_beta_tm_it0_{0}seeds.pdf'.format(eseed))

#plot of the mean: tm without svd
fig19, ax19=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax19[0].plot(xseed6, ave_beta_x6, ".", label='tm success')
ax19[0].plot(xseed7, ave_beta_x7, ".", label='tm all')
ax19[0].set_ylabel("mean$_x$ [%]")
ax19[0].legend(fontsize=8,loc='best')
ax19[1].plot(xseed6, ave_beta_y6, ".", label='tm success')
ax19[1].plot(xseed7, ave_beta_y7, ".", label='tm all')
ax19[1].set_xlabel("seed")
ax19[1].set_ylabel("mean$_y$ [%]")
ax19[1].legend(fontsize=8,loc='best')
fig19, plt.subplots_adjust(left=.16, right=.97, top=.94, bottom=.11)
fig19.savefig(path+'mean_beta_tm_it0_{0}seeds.pdf'.format(eseed))

#plot of the rms: tm with svd
fig23, ax23=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax23[0].plot(xseed8, rms_beta_x8, ".", label='tm success')
ax23[0].plot(xseed9, rms_beta_x9, ".", label='tm all')
ax23[0].set_ylabel("mean$_x$ [%]")
ax23[0].legend(fontsize=8,loc='best')
ax23[1].plot(xseed8, rms_beta_y8, ".", label='tm success')
ax23[1].plot(xseed9, rms_beta_y9, ".", label='tm all')
...
fig23.savefig(path+'rms_beta_tm_it1_{0}seeds.pdf'.format(eseed))

#plot of the mean: tm with svd
fig24, ax24=plt.subplots(nrows=2, ncols=1, sharex=True, sharey=False)
ax24[0].plot(xseed6, ave_beta_x6, ".", label='tm success')
ax24[0].plot(xseed7, ave_beta_x7, ".", label='tm all')
ax24[0].set_ylabel("mean$_x$ [%]")
ax24[0].legend(fontsize=8,loc='best')
ax24[1].plot(xseed6, ave_beta_y6, ".", label='tm success')
ax24[1].plot(xseed7, ave_beta_y7, ".", label='tm all')
...
fig24.savefig(path+'mean_beta_tm_it1_{0}seeds.pdf'.format(eseed))
...
fig1.tight_layout()
fig1.savefig(path+'hist_beta_line_sextuoff_it1_{0}seeds.pdf'.format(eseed))
...
fig8.tight_layout()
fig8.savefig(path+'hist_beta_line_sextuoff_it2_{0}seeds.pdf'.format(eseed))
...
fig14.tight_layout()
fig14.savefig(path+'hist_beta_ring_sextuoff_{0}seeds.pdf'.format(eseed))
...
fig15.tight_layout()
fig15.savefig(path+'hist_beta_ring_sextuoff_last_it_{0}seeds.pdf'.format(eseed))
