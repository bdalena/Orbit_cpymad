...
dfan["rms_x"] = mean_corr_x_all_seed*Brho
dfan["rms_y"] = mean_corr_y_all_seed*Brho
...
print('\n')
dfan.to_csv('corr_rms_analy.dat', sep=' ',)
data = pd.DataFrame()
for i in range(eseed):
    ...
    if os.path.exists(path+"test_seed{0}/FCCee_heb_modett_orbcor_all_line_sextuoff_it2_seed{0}.tfs".format(i+1)):
        ...
        optics_allx_val = optics_allx['PX.OLD'][optics_allx['PX.OLD'] !=0.0]
        optics_allx_valc = optics_allx['PX.CORRECTION'][optics_allx['PX.CORRECTION'] !=0.0]
        print(optics_allx_val.std())

        rms_dist_x_it1=np.append(rms_dist_x_it1,optics_allx_val.std()*Brho)
        ave_dist_x_it1=np.append(ave_dist_x_it1,optics_allx_val.mean()*Brho)
        rms_dist_x_it2=np.append(rms_dist_x_it2,optics_allx_valc.std()*Brho)
        ave_dist_x_it2=np.append(ave_dist_x_it2,optics_allx_valc.mean()*Brho)
        ...
        optics_ally_val = optics_ally['PY.OLD'][optics_ally['PY.OLD'] !=0.0]
        optics_ally_valc = optics_ally['PY.CORRECTION'][optics_ally['PY.CORRECTION'] !=0.0]

        rms_dist_y_it1=np.append(rms_dist_y_it1,optics_ally_val.std()*Brho)
        ave_dist_y_it1=np.append(ave_dist_y_it1,optics_ally_val.mean()*Brho)
        rms_dist_y_it2=np.append(rms_dist_y_it2,optics_ally_valc.std()*Brho)
        ave_dist_y_it2=np.append(ave_dist_y_it2,optics_ally_valc.mean()*Brho)
    ...
    if os.path.exists(path+"test_seed{0}/FCCee_heb_modett_tune_match_it1_seed{0}.tfs".format(i+1)):
        ...
        optics_allx4_val = optics_allx4['PX.OLD'][optics_allx4['PX.OLD'] !=0.0]
        optics_allx4_valc = optics_allx4['PX.CORRECTION'][optics_allx4['PX.CORRECTION'] !=0.0]
        rms_dist_x_it6=np.append(rms_dist_x_it6,optics_allx4_val.std()*Brho)
        ave_dist_x_it6=np.append(ave_dist_x_it6,optics_allx4_val.mean()*Brho)
        rms_dist_x_it7=np.append(rms_dist_x_it7,optics_allx4_valc.std()*Brho)
        ave_dist_x_it7=np.append(ave_dist_x_it7,optics_allx4_valc.mean()*Brho)
        ...
        optics_allx4_val = optics_ally4['PY.OLD'][optics_ally4['PY.OLD']!=0.0]
        optics_allx4_valc = optics_ally4['PY.CORRECTION'][optics_ally4['PY.CORRECTION']!=0.0]
        rms_dist_y_it6=np.append(rms_dist_y_it6,optics_ally4_val.std()*Brho)
        ave_dist_y_it6=np.append(ave_dist_y_it6,optics_ally4_val.std()*Brho)
        rms_dist_y_it7=np.append(rms_dist_y_it7,optics_ally4_valc.std()*Brho)
        ave_dist_y_it7=np.append(ave_dist_y_it7,optics_ally4_valc.mean()*Brho)
...
data.to_csv('corr_values.dat', sep=' ',)

print('numerical rms x = ',rms_dist_x_it4[0])
print('numerical rms y = ',rms_dist_y_it4[0])
print('\n')

mean_rms_dist_x2=np.mean(rms_dist_x_it4)
mean_rms_dist_y2=np.mean(rms_dist_y_it4)
max_rms_dist_x2=np.max(rms_dist_x_it4)
max_rms_dist_y2=np.max(rms_dist_y_it4)

print('numerical rms: mean_rms_dist_x2 = ',mean_rms_dist_x2)
print('numerical rms: mean_rms_dist_y2 = ',mean_rms_dist_y2)
print('numerical rms: max_rms_dist_x2 = ',max_rms_dist_x2)
print('numerical rms: max_rms_dist_y2 = ',max_rms_dist_y2)
...
if os.path.exists(path+"test_seed{0}/FCCee_heb_modett_orbcor_all_ring_sextuon_it1_seed{0}.tfs".format(eseed)):
    ...
    ax11[1].axhline(y=max_corr_y, color='g', linestyle='--', label='analytical rms')
...
print("Successful seeds MS off last it = {}".format(len(rms_dist_x_it4)))
print("Successful seeds MS on it1 = {}".format(len(rms_dist_x_it5)))
